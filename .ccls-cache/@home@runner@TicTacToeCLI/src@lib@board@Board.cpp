...
public:
  ...
  short getCurrentRound() { return currentRound; };
  string getCurrentTurn() { return currentTurn; };

  bool checkVertVictoryForEachCol(string player) {
    return (board[0][0] == player && board[1][0] == player &&
            board[2][0] == player) or
  ...
  bool checkDiagonalVictory(string player) {
    return (board[0][2] == player && board[1][1] == player &&
            board[2][0] == player) or
